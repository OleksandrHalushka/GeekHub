...
def dictionary_cleaner(some_dictionary):
    ...
    for index in our_keys:
        if some_dictionary[index] in values:
            continue
        else:
            values.append(some_dictionary[index])
            cleaned_dictionary[index] = some_dictionary[index]
    return cleaned_dictionary


if __name__ == '__main__':
    strange_dictionary = {'a': 1, 'b': 3, 'c': 6, 'd': 8, 'e': 9, 'f': 11, 'g': 3, 'h': 4, 'i': 5, 'j': 4, 'k': 2,
                          'l': 3, 'm': 1, 'n': 8, 'o': 5, 'p': 9, 'q': 4, 'r': 4, 's': 6, 't': 4, 'y': 1}
